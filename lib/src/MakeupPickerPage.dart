...
import 'package:flutter_login_signup/src/DressingPickerPage.dart';
...
class MakeupPickerPage extends StatefulWidget {
  double budget;
  String title;
  String date;
  String eventType;
  ...
  MakeupPickerPage( String title,String date,double budget, String eventType,eventLocationValue,eventMusicValue , eventPhotographerValue  ){
  ...
  @override
  _PageState createState() => _PageState(budget,title,date,eventType,eventLocationValue,eventMusicValue,eventPhotographerValue);
}

class _PageState extends State<MakeupPickerPage> {
  Config c = new Config();
  ...
  _PageState(double budget, title, date, eventType, eventLocationValue, eventMusicValue , eventPhotographerValue){
    this.budget=budget;
    this.title=title;
    this.eventType=eventType;
    this.date=date;
  ...
  _updateOptionAfterSelect() async{
    ...
    Map res = await events.getEventMakeupList(eventType);
    if(res['success']==true){
      ...
      for(int i=0;i<res['beautys'].length;i++  ){
        if(res['beautys'][i]['price'] < budget ){
          _tmp.add(
            RadioListTile(
              title: Text(res['beautys'][i]['name']),
              value: res['beautys'][i]['_id'].toString(),
      ...
      for(int i=0;i<res['beautys'].length;i++){
        if(res['beautys'][i]['key']==_eventLocationValue){
          setState(() {
            _loactionMap=res['beautys'][i];
          });
          break;
        }
      }
  ...
  _getEventLocationsList() async{
    ...
    if(res['success']==true){
      setState(() {
        _eventLocationValue=res['beautys'][0]['_id'].toString();
        _loactionMap=res['beautys'][0];
      ...
      for(int i=0;i<res['beautys'].length;i++  ){
        if(res['beautys'][i]['price'] < budget ){
          _tmp.add(
            RadioListTile(
              title: Text(res['beautys'][i]['name'].toString()),
              value: res['beautys'][i]['_id'].toString(),
              groupValue: _eventLocationValue,
              onChanged: (String data) async {
                _eventLocationValue=data;
    ...
    return GestureDetector(
        ...
        onTap: () async {
          Navigator.push(context,
              MaterialPageRoute(builder: (context) => DressingPickerPage(title,date,( budget - _loactionMap['price'] ),eventType,eventLocationValue,eventMusicValue,eventPhotographerValue,_eventLocationValue  ))) ;
                    ...
                    Container(
                      ...
                      child: Column(
                        ...
                        children: <Widget>[
                          ...
                          Text('What about some makeup ?',style: TextStyle(fontWeight: FontWeight.w700,fontSize: 18),),
