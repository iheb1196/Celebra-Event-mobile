...
class DressingPickerPage extends StatefulWidget {
  double budget;
  String title;
  String date;
  String eventType;
  String eventLocationValue;
  String eventMusicValue;
  String eventPhotographerValue;
  String eventMakeupValue;



  DressingPickerPage( String title,String date,double budget, String eventType,eventLocationValue,eventMusicValue , eventPhotographerValue ,eventMakeupValue  ){
    ...
    this.eventMusicValue=eventMusicValue;
    this.eventPhotographerValue=eventPhotographerValue;
    this.eventMakeupValue=eventMakeupValue;
  }


  @override
  _PageState createState() => _PageState(budget,title,date,eventType,eventLocationValue,eventMusicValue,eventPhotographerValue,eventMakeupValue);
}

class _PageState extends State<DressingPickerPage> {
  Config c = new Config();
  ...
  _PageState(double budget, title, date, eventType, eventLocationValue, eventMusicValue , eventPhotographerValue,eventMakeupValue){
    this.budget=budget;
    this.title=title;
    this.eventType=eventType;
    this.date=date;
  ...
  _updateOptionAfterSelect() async{
    ...
    Map res = await events.getEventDressingList(eventType);
    if(res['success']==true){
      ...
      for(int i=0;i<res['wearing'].length;i++  ){
        if(res['wearing'][i]['price'] < budget ){
          _tmp.add(
            RadioListTile(
              title: Text(res['wearing'][i]['name']),
              value: res['wearing'][i]['_id'].toString(),
      ...
      for(int i=0;i<res['wearing'].length;i++){
        if(res['wearing'][i]['key']==_eventLocationValue){
          setState(() {
            _loactionMap=res['wearing'][i];
          });
          break;
        }
      }
  ...
  _getEventLocationsList() async{
    ...
    if(res['success']==true){
      setState(() {
        _eventLocationValue=res['wearing'][0]['_id'].toString();
        _loactionMap=res['wearing'][0];
      ...
      for(int i=0;i<res['wearing'].length;i++  ){
        if(res['wearing'][i]['price'] < budget ){
          _tmp.add(
            RadioListTile(
              title: Text(res['wearing'][i]['name'].toString()),
              value: res['wearing'][i]['_id'].toString(),
              groupValue: _eventLocationValue,
              onChanged: (String data) async {
                _eventLocationValue=data;
  ...
  Widget _submitButton(c) {
    return GestureDetector(
        ...
        onTap: () async {
          ...
          events.createEvent(title,eventType,date,eventLocationValue,eventMusicValue,eventPhotographerValue,eventMakeupValue,_eventLocationValue).then((res){
            pr.hide();
                    ...
                    Container(
                      ...
                      child: Column(
                        ...
                        children: <Widget>[
                          ...
                          Text('All we need now is a good dress \n& suit  ?',style: TextStyle(fontWeight: FontWeight.w700,fontSize: 18),),
