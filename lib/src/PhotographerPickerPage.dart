...
import 'package:flutter_login_signup/src/MakeupPickerPage.dart';
...
class PhotographerPickerPage extends StatefulWidget {
  double budget;
  String title;
  String date;
  String eventType;
  ...
  PhotographerPickerPage( String title,String date,double budget, String eventType,eventLocationValue,eventMusicValue  ){
  ...
  @override
  _PhotographerPickerPageState createState() => _PhotographerPickerPageState(budget,title,date,eventType,eventLocationValue,eventMusicValue);
}

class _PhotographerPickerPageState extends State<PhotographerPickerPage> {
  Config c = new Config();
  ...
  _PhotographerPickerPageState(double budget, title, date, eventType, eventLocationValue, eventMusicValue){
    this.budget=budget;
    this.title=title;
    this.eventType=eventType;
    this.date=date;
  ...
  _updateOptionAfterSelect() async{
    ...
    Map res = await events.getEventPhotographgerList(eventType);
    if(res['success']==true){
      ...
      for(int i=0;i<res['photographs'].length;i++  ){
        if(res['photographs'][i]['price'] < budget ){
          _tmp.add(
            RadioListTile(
              title: Text(res['photographs'][i]['name']),
              value: res['photographs'][i]['_id'].toString(),
      ...
      for(int i=0;i<res['photographs'].length;i++){
        if(res['photographs'][i]['key']==_eventLocationValue){
          setState(() {
            _loactionMap=res['photographs'][i];
          });
          break;
        }
      }
  ...
  _getEventLocationsList() async{
    ...
    if(res['success']==true){
      setState(() {
        _eventLocationValue=res['photographs'][0]['_id'].toString();
        _loactionMap=res['photographs'][0];
      ...
      for(int i=0;i<res['photographs'].length;i++  ){
        if(res['photographs'][i]['price'] < budget ){
          _tmp.add(
            RadioListTile(
              title: Text(res['photographs'][i]['name'].toString()),
              value: res['photographs'][i]['_id'].toString(),
              groupValue: _eventLocationValue,
              onChanged: (String data) async {
                _eventLocationValue=data;
  ...
  Widget _submitButton(c) {
    return GestureDetector(
        ...
        onTap: () async {
              ...
              MaterialPageRoute(builder: (context) => MakeupPickerPage(title,date,( budget - _loactionMap['price'] ),eventType,eventLocationValue,eventMusicValue,_eventLocationValue  ))) ;
                    ...
                    Container(
                      ...
                      child: Column(
                        ...
                        children: <Widget>[
                          ...
                          Text('What about the photographer ?',style: TextStyle(fontWeight: FontWeight.w700,fontSize: 18),),
